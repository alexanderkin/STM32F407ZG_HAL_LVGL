...
bool TouchPad_Is_Press = false;
int16_t Current_TouchPad_Pos[2];
...
static int32_t GTP_I2C_Test(void);
...
static int I2C_Transfer( struct i2c_msg *msgs,int num)
{
	...
	for (i = 0; ret == 0 && i != num; i++) {
		if ((msgs[i].flags & I2C_M_RD))     //根据flag判断是读数据还是写数据
			ret = I2C_ReadBytes(msgs[i].addr, msgs[i].buf, msgs[i].len);	//IIC读取数据
		else
			ret = I2C_WriteBytes(msgs[i].addr, msgs[i].buf, msgs[i].len);	//IIC写入数据
	}

	if(ret)
		return ret;

	return i;   //正常完成的传输结构个数
}
...
static void GTP_Touch_Down(int32_t id, int32_t x, int32_t y, int32_t w)
{
    ...
    pre_x[id] = x;
    pre_y[id] = y;
    Current_TouchPad_Pos[0] = x;
    Current_TouchPad_Pos[1] = y;
}
...
static void GTP_Touch_Up(int32_t id)
{
	...
	TouchPad_Is_Press = false;

    /*触笔释放，把pre xy 重置为负*/
	pre_x[id] = -1;
	pre_y[id] = -1;
    Current_TouchPad_Pos[0] = -1;
    Current_TouchPad_Pos[1] = -1;
...
static void Goodix_TS_Work_Func(void)
{
    ...
    uint8_t touch_num   = 0, finger = 0, id = 0;
...
{
    int32_t ret = -1, i = 0, retry = 0;
    uint16_t check_sum = 0;

    const uint8_t* cfg_info;
    uint8_t cfg_info_len, cfg_num =0;   //需要配置的寄存器个数
	uint8_t* config;
